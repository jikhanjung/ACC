...
**변경 후**: 3단계 프로세스
1. **Step 1**: Local Matrix 로드 → Dendrogram 시각화
2. **Step 2**: Global Matrix 로드 → Dendrogram 시각화
...
- `sim_local` 정렬 시 단일 멤버가 먼저 와서 알고리즘 오작동
...
clusters = extract_clusters_from_dendro(local_dendro)  # 단일 멤버 포함!

# 수정 후 (정상 작동)
acc_result = build_acc_from_matrices(local_matrix, global_matrix)
...
### After
1. **Step 1**: Local CSV 로드 → Dendrogram 확인 가능
2. **Step 2**: Global CSV 로드 → Dendrogram 확인 가능
...
```bash
$ python acc_gui.py
✓ Local dendrogram: 표시됨
✓ Global dendrogram: 표시됨
