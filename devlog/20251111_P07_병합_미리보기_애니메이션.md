# 20251111 P07: 병합 미리보기 애니메이션

## 개요

Next 버튼 클릭 시 병합될 클러스터를 미리 하이라이트로 보여주는 애니메이션 기능 추가.

## 사용자 요청

> "Next 버튼을 눌렀을 때 먼저 합쳐질 area 의 row/column 을 highlight 해서 잠시(0.5초?) 보여주고 합쳐주면 좋겠어."

### 목표

1. Next 버튼 클릭 시 **즉시 병합하지 않음**
2. **미리보기 단계**: 병합될 클러스터를 빨간색으로 하이라이트
3. **0.5초 대기** 후 실제 병합 수행
4. 사용자가 어떤 클러스터가 병합되는지 명확히 인지

## 구현

### 1. 새로운 상태 추가 (StepMatrixWidget)

```python
class StepMatrixWidget(QWidget):
    def __init__(self, title="Matrix", parent=None):
        # ...
        self.is_preview_mode = False  # 미리보기 모드 여부
        self.preview_clusters = None  # (idx_i, idx_j) 미리보기 대상
```

### 2. Next 버튼 동작 수정

#### Before (즉시 병합)
```python
def next_step(self):
    if self.current_step < max_step:
        self.step_slider.setValue(self.current_step + 1)
```

#### After (미리보기 → 병합)
```python
def next_step(self):
    """Go to next step (with preview animation)"""
    if self.step_manager and self.current_step < self.step_manager.get_num_steps() - 1:
        # 1. 다음 단계 정보 가져오기
        next_step_num = self.current_step + 1
        next_step_info = self.step_manager.get_step(next_step_num)

        if next_step_info and next_step_info['merged_pair']:
            # 2. 병합될 클러스터 찾기
            cluster_i, cluster_j = next_step_info['merged_pair']

            # 3. 현재 단계에서 해당 클러스터의 인덱스 찾기
            current_step_info = self.step_manager.get_step(self.current_step)
            current_labels = current_step_info['labels']

            idx_i = self._find_cluster_index(current_labels, cluster_i)
            idx_j = self._find_cluster_index(current_labels, cluster_j)

            if idx_i is not None and idx_j is not None:
                # 4. 미리보기 모드 활성화
                self.is_preview_mode = True
                self.preview_clusters = (idx_i, idx_j)
                self.update_step_display()  # 빨간색으로 하이라이트

                # 5. 500ms 후 실제 병합
                QTimer.singleShot(500, self._complete_next_step)
            else:
                # Fallback: 바로 병합
                self._complete_next_step()
        else:
            # 병합 정보 없음: 바로 이동
            self._complete_next_step()
```

### 3. 헬퍼 메서드 추가

#### 클러스터 인덱스 찾기
```python
def _find_cluster_index(self, labels, cluster):
    """Find the index of a cluster in labels list"""
    cluster_set = set(cluster)
    for idx, label in enumerate(labels):
        if isinstance(label, (tuple, list)):
            # 병합된 클러스터 (tuple/list)
            if set(label) == cluster_set:
                return idx
        else:
            # 단일 클러스터
            if {label} == cluster_set:
                return idx
    return None
```

#### 병합 완료
```python
def _complete_next_step(self):
    """Complete the transition to next step"""
    self.is_preview_mode = False
    self.preview_clusters = None
    self.step_slider.setValue(self.current_step + 1)
```

### 4. 테이블 하이라이트 로직 수정

#### populate_table 메서드

```python
def populate_table(self, matrix, labels):
    # ... 테이블 설정 ...

    # 하이라이트 결정
    if self.is_preview_mode and self.preview_clusters:
        # 미리보기 모드: 병합될 클러스터 (빨간색)
        highlight_indices = self.preview_clusters
        highlight_color = Qt.GlobalColor.red
    else:
        # 일반 모드: 이미 병합된 클러스터 (노란색)
        merged_idx = -1
        if self.current_step > 0:
            for idx, label in enumerate(labels):
                if isinstance(label, (tuple, list)):
                    merged_idx = idx
                    break
        highlight_indices = (merged_idx,) if merged_idx >= 0 else ()
        highlight_color = Qt.GlobalColor.yellow

    # 테이블 채우기 (upper triangle만)
    for i in range(n):
        for j in range(n):
            if i < j:
                # Upper triangle
                value = matrix[i, j]
                item = QTableWidgetItem(f"{value:.3f}")

                # 하이라이트 적용
                should_highlight = False
                if self.is_preview_mode and self.preview_clusters:
                    # 미리보기: 두 클러스터 모두 하이라이트
                    idx_i, idx_j = self.preview_clusters
                    if i == idx_i or i == idx_j or j == idx_i or j == idx_j:
                        should_highlight = True
                elif highlight_indices:
                    # 일반: 병합된 클러스터만 하이라이트
                    if any(i == idx or j == idx for idx in highlight_indices):
                        should_highlight = True

                if should_highlight:
                    item.setBackground(highlight_color)

                self.table.setItem(i, j, item)
            # ... diagonal, lower triangle 처리 ...
```

## 동작 흐름

### Step 0 → Step 1 전환 예시

```
현재 상태 (Step 0):
       J    T    Y    N    O    Q
J      -   0.9  0.8  0.4  0.35 0.36
T          -    0.8  0.38 0.33 0.34
Y               -    0.37 0.32 0.33
...

1. 사용자: Next 버튼 클릭

2. 시스템: 다음 단계 확인
   - Step 1에서 병합: (J) + (T)
   - 현재 labels: ['J', 'T', 'Y', 'N', 'O', 'Q']
   - idx_i = 0 (J의 인덱스)
   - idx_j = 1 (T의 인덱스)

3. 미리보기 모드 활성화 (0.0초)
       J    T    Y    N    O    Q
J      -   🔴  🔴  🔴  🔴  🔴   ← J 행 빨간색
T      🔴   -   🔴  🔴  🔴  🔴   ← T 행 빨간색
Y      🔴  🔴   -    0.37 0.32 0.33
N      🔴  🔴  0.37  -   0.75 0.75
...
       ↑   ↑
     J열 T열도 빨간색

4. 0.5초 대기 (사용자가 확인)

5. 병합 완료 (0.5초 후)
       J+T  Y    N    O    Q
J+T     -   0.8  0.39 0.34 0.35  ← 병합 완료, 노란색
Y      0.8  -    0.37 0.32 0.33
N      0.39 0.37  -   0.75 0.75
...
```

## 색상 구분

| 상태 | 색상 | 의미 |
|------|------|------|
| 미리보기 | 🔴 빨간색 | 다음에 병합될 클러스터 |
| 병합 완료 | 🟡 노란색 | 현재 단계에서 병합된 클러스터 |
| 일반 | ⬜ 흰색 | 병합되지 않은 일반 클러스터 |
| 대각선 | ⬛ 회색 | 대각선 (숨김) |

## 사용자 경험 개선

### Before (개선 전)
```
사용자: Next 클릭
시스템: 즉시 병합
사용자: "어? 뭐가 합쳐진 거지? 🤔"
```

### After (개선 후)
```
사용자: Next 클릭
시스템: J와 T를 빨간색으로 하이라이트 (0.5초)
사용자: "아, J와 T가 합쳐지는구나! 👀"
시스템: 병합 완료 (J+T)
사용자: "이해했어! ✅"
```

## 기술적 특징

### 1. QTimer.singleShot 사용
```python
QTimer.singleShot(500, self._complete_next_step)
```
- **비동기 처리**: GUI가 freeze되지 않음
- **정확한 타이밍**: 500ms 후 정확히 실행
- **깔끔한 코드**: 콜백 패턴으로 간단하게 구현

### 2. 상태 관리
```python
self.is_preview_mode = False  # 초기 상태
self.preview_clusters = None  # 초기 상태

# 미리보기 활성화
self.is_preview_mode = True
self.preview_clusters = (idx_i, idx_j)

# 병합 완료 후 초기화
self.is_preview_mode = False
self.preview_clusters = None
```
- **명확한 상태**: 미리보기/일반 모드 구분
- **안전한 초기화**: 병합 완료 후 반드시 초기화

### 3. Prev 버튼 처리
```python
def prev_step(self):
    """Go to previous step (immediate, no preview)"""
    if self.current_step > 0:
        self.is_preview_mode = False  # 미리보기 취소
        self.preview_clusters = None
        self.step_slider.setValue(self.current_step - 1)
```
- Prev는 **미리보기 없음** (즉시 이동)
- 이유: 이전 단계는 이미 본 것이므로 불필요

## 향후 개선 가능성

### 1. 애니메이션 효과
```python
# Fade in/out 효과
for alpha in range(0, 255, 25):
    color = QColor(255, 0, 0, alpha)  # 투명 → 불투명
    item.setBackground(color)
    QTimer.singleShot(alpha // 25 * 50, update_table)
```

### 2. 타이밍 사용자 설정
```python
# 설정에서 대기 시간 조정
preview_delay_ms = settings.get('preview_delay', 500)
QTimer.singleShot(preview_delay_ms, self._complete_next_step)
```

### 3. 소리 효과
```python
# 미리보기 시작 시 "띵" 소리
QSound.play("sounds/preview.wav")

# 병합 완료 시 "딸깍" 소리
QSound.play("sounds/merge.wav")
```

### 4. 진행 바 표시
```python
# 0.5초 동안 진행 바 표시
progress_bar.setMaximum(500)
for ms in range(0, 500, 50):
    QTimer.singleShot(ms, lambda: progress_bar.setValue(ms))
```

## 테스트 시나리오

### 1. 정상 동작
1. GUI 실행
2. sample_subordinate.csv 로드
3. Next 버튼 클릭
4. ✅ J, T 행/열이 빨간색으로 하이라이트
5. ✅ 0.5초 후 J+T로 병합
6. ✅ J+T 행/열이 노란색으로 하이라이트

### 2. 연속 클릭
1. Next 클릭
2. 0.5초 안에 다시 Next 클릭
3. ✅ 두 번째 클릭 무시 (QTimer 진행 중)
4. ✅ 첫 번째 병합 완료 후 다음 클릭 가능

### 3. Prev 중단
1. Next 클릭 (미리보기 시작)
2. 0.5초 안에 Prev 클릭
3. ✅ 미리보기 취소
4. ✅ 이전 단계로 즉시 이동

## 결론

미리보기 애니메이션 추가로:
- ✅ 사용자가 병합 과정을 명확히 이해
- ✅ 시각적 피드백으로 직관성 향상
- ✅ 학습/교육 효과 증가
- ✅ 0.5초 딜레이로 과정 관찰 가능
- ✅ QTimer로 깔끔하게 구현

**클러스터링 과정을 더 쉽게 이해할 수 있습니다!**
