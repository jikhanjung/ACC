...
@pytest.fixture
def sample_local_matrix(test_data_dir):
    """샘플 local matrix (6-영역)"""
    df = pd.read_csv(test_data_dir / "sample_local.csv", index_col=0)
    return {idx: dict(row) for idx, row in df.iterrows()}

@pytest.fixture
def sample_global_matrix(test_data_dir):
    """샘플 global matrix (6-영역)"""
    df = pd.read_csv(test_data_dir / "sample_global.csv", index_col=0)
    return {idx: dict(row) for idx, row in df.iterrows()}
...
#### Phase 1: 준비 및 인프라 구축 ✅
- tests/ 디렉토리 구조 생성 (unit, integration, gui, performance)
- pytest 설정 파일 작성 (pytest.ini)
- 개발 의존성 파일 작성 (requirements-dev.txt)
- Fixture 파일 작성 (conftest.py, 10개 fixtures)
- 테스트 데이터 복사 (tests/test_data/)
...
#### Phase 5: CI/CD 및 마무리 ✅
- GitHub Actions workflow 작성 (.github/workflows/test.yml)
- README에 테스트 배지 추가 (Tests, Codecov)
- Python 3.9-3.12 멀티버전 테스트 지원
- Codecov 통합

### 테스트 통계

```
테스트 파일: 4개 (unit: 3, integration: 1)
테스트 케이스: 71개 (모두 통과)
실행 시간: ~5.8초
전체 커버리지: 22%
...
├── unit/
│   ├── __init__.py
│   ├── test_acc_core.py        # 17 tests
│   ├── test_acc_core_new.py    # 26 tests
│   └── test_acc_utils.py       # 15 tests
├── integration/
│   ├── __init__.py
...
2. `[새로 생성]` - Add pytest-based test suite and reorganize tests (P28)
3. `[새로 생성]` - Add comprehensive unit tests for acc_core and acc_core_new (P28)

### 미완료 항목

#### Phase 4: GUI 테스트 (보류)
- GUI 테스트는 PyQt6 의존성 및 환경 설정 복잡성으로 인해 보류
- 향후 필요 시 pytest-qt를 사용하여 추가 예정

#### 커버리지 개선 영역
- acc_core.py의 병합 함수 (add_area_to_cluster, merge_two_clusters)
- acc_core_new.py의 iterative algorithm 함수
- acc_core_acc2.py (0% - 사용 여부 확인 필요)
- GUI 코드 (acc_gui.py, 0%)

### 향후 개선 방향

1. **커버리지 향상**
   - 병합 알고리즘 테스트 추가로 acc_core.py 커버리지를 80%+로
   - acc_core_new.py의 iterative functions 테스트 추가

2. **GUI 테스트**
   - pytest-qt 설정 완료 후 기본 GUI 동작 테스트 추가
   - Snapshot 테스트로 시각화 출력 검증

3. **성능 테스트**
   - tests/performance/ 디렉토리에 벤치마크 테스트 추가
   - 대규모 데이터셋 처리 성능 측정

4. **기존 테스트 파일 정리**
   - 루트의 17개 테스트 파일을 tests/legacy/ 또는 examples/로 이동
   - 유용한 테스트는 새 구조로 통합

### 결론

✅ **핵심 목표 달성**: pytest 기반 테스트 스위트 구축 완료
✅ **품질 확보**: 71개 테스트로 핵심 알고리즘 70%+ 커버리지
✅ **자동화**: GitHub Actions CI/CD 파이프라인 구축 완료
📊 **개선 여지**: 전체 커버리지 22% → 향후 병합/GUI 테스트 추가 필요

테스트 체계화 작업을 통해 ACC 프로젝트의 코드 품질과 유지보수성이 크게 향상되었습니다.
