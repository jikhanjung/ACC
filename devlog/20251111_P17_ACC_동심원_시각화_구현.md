# 20251111 P17: ACC 동심원 시각화 구현

## 개요

ACC 알고리즘의 핵심 개념인 **동심원(concentric circles)** 구조를 올바르게 구현.

## 문제

### Before (틀림)

```
하나의 원에 모든 area 표시:
       Y
   J───+───T   O───Q  N    ← 모든 점이 같은 원 위
       ⭕

문제:
- ❌ 단일 원만 표시
- ❌ 각 클러스터의 고유한 diameter 정보 손실
- ❌ 논문의 핵심 개념 미구현
```

### After (올바름)

```
여러 개의 동심원, 클러스터별로 분리:

    ⭕⭕⭕⭕⭕  ← Cluster 5 (d=1.818, 가장 큰 원)
     ⭕⭕⭕⭕   ← Cluster 4 (d=1.429)
      ⭕⭕⭕    ← Cluster 3 (d=1.282)
       ⭕⭕     ← Cluster 2 (d=1.205)
        ⭕      ← Cluster 1 (d=1.136, 가장 작은 원)

개선:
- ✅ 여러 개의 원 표시
- ✅ 각 클러스터가 고유한 diameter 가짐
- ✅ sim_global 기반 diameter 시각화
- ✅ 논문의 동심원 개념 구현
```

## 구현

### 1. acc_core.py - build_acc() 수정

**Before (단일 병합)**

```python
def build_acc(local_dendro, global_dendro, global_matrix, unit=1.0):
    # 1) 모든 클러스터 추출
    clusters = extract_clusters_from_dendro(local_dendro)

    # 2) 각 클러스터에 sim_global, diameter, theta 부여
    decorate_clusters(clusters, global_dendro, global_matrix, unit=unit)

    # 3) 유사도 순 정렬
    clusters.sort(key=lambda c: c["sim_local"], reverse=True)

    # 4) 첫 클러스터 배치
    base = place_first_cluster(clusters[0])

    # 5) 나머지 클러스터를 하나씩 병합 ⚠️
    for c in clusters[1:]:
        if ...:
            base = add_area_to_cluster(base, c, global_matrix)
        else:
            base = merge_two_clusters(base, c, global_matrix)

    # ⚠️ 단일 결과 반환
    return base
```

**After (다중 동심원)**

```python
def build_acc(local_dendro, global_dendro, global_matrix, unit=1.0):
    """
    Build ACC result with multiple concentric circles
    Each cluster gets its own circle based on its diameter

    Returns:
        dict with:
            - 'clusters': list of positioned clusters
            - 'all_members': set of all members across all clusters
    """
    # 1) 모든 클러스터 추출
    clusters = extract_clusters_from_dendro(local_dendro)

    # 2) 각 클러스터에 sim_global, diameter, theta 부여
    decorate_clusters(clusters, global_dendro, global_matrix, unit=unit)

    # 3) 단일 멤버 클러스터 제외 (2개 이상만)
    clusters = [c for c in clusters if len(c["members"]) >= 2]

    # 4) 유사도 높은 순으로 정렬
    clusters.sort(key=lambda c: c["sim_local"], reverse=True)

    # 5) 각 클러스터를 독립적으로 배치 ✅
    positioned_clusters = []
    for c in clusters:
        positioned = place_first_cluster(c)
        positioned_clusters.append(positioned)

    # 6) 모든 클러스터 반환 ✅
    all_members = set()
    for c in positioned_clusters:
        all_members.update(c["members"])

    return {
        "clusters": positioned_clusters,
        "all_members": all_members
    }
```

**핵심 변경:**
- ❌ Before: 모든 클러스터를 **병합**하여 하나로
- ✅ After: 각 클러스터를 **독립적으로 배치**

### 2. acc_utils.py - build_acc_from_matrices() 간소화

**Before**

```python
def build_acc_from_matrices(local_matrix, global_matrix, unit=1.0, method='average'):
    # ... 덴드로그램 생성

    # 클러스터 추출 및 병합 로직 중복 구현 ⚠️
    clusters = extract_clusters_from_dendro_filtered(local_dendro)
    decorate_clusters(clusters, global_dendro, global_matrix, unit=unit)
    clusters.sort(...)
    base = place_first_cluster(clusters[0])
    for c in clusters[1:]:
        # 병합 로직
        ...

    return base
```

**After**

```python
def build_acc_from_matrices(local_matrix, global_matrix, unit=1.0, method='average'):
    """
    Build ACC result directly from similarity matrices with multiple concentric circles
    """
    from acc_core import build_acc

    # 덴드로그램 생성
    local_dendro, sub_labels = matrix_to_dendrogram(local_matrix, method=method)
    global_dendro, inc_labels = matrix_to_dendrogram(global_matrix, method=method)

    # build_acc 호출 (이제 다중 동심원 반환)
    acc_result = build_acc(local_dendro, global_dendro, global_matrix, unit=unit)

    return acc_result
```

**개선:**
- ✅ 중복 코드 제거
- ✅ `build_acc()` 함수에 위임
- ✅ 코드 간소화 (45줄 → 12줄)

### 3. acc_gui.py - plot_acc_result() 수정

**Before (단일 원)**

```python
def plot_acc_result(self, acc_result):
    # 단일 결과에서 데이터 추출
    center = acc_result.get("center", (0, 0))
    diameter = acc_result.get("diameter", 1.0)
    radius = diameter / 2.0
    points = acc_result.get("points", {})
    members = acc_result.get("members", set())

    # 원 하나만 그리기 ⚠️
    circle = plt.Circle(center, radius, fill=False,
                       edgecolor='blue', linewidth=2)
    ax.add_patch(circle)

    # 모든 점을 같은 원 위에 표시
    for member, (x, y) in points.items():
        ax.plot(x, y, 'o', markersize=10, ...)
```

**After (다중 동심원)**

```python
def plot_acc_result(self, acc_result):
    """Plot ACC result with multiple concentric circles"""
    # 다중 클러스터에서 데이터 추출
    clusters = acc_result.get("clusters", [])
    all_members = acc_result.get("all_members", set())

    if len(clusters) == 0:
        # 에러 표시
        return

    # 클러스터별 색상 생성
    cluster_colors = plt.cm.Set3(np.linspace(0, 1, len(clusters)))

    # 중복 방지를 위한 추적
    plotted_members = set()

    # 각 클러스터마다 원 그리기 ✅
    for cluster_idx, cluster in enumerate(clusters):
        center = cluster.get("center", (0, 0))
        diameter = cluster["diameter"]
        radius = diameter / 2.0
        points = cluster["points"]
        members = cluster["members"]

        # 클러스터의 원 그리기
        circle = plt.Circle(center, radius, fill=False,
                          edgecolor=cluster_colors[cluster_idx],
                          linewidth=2,
                          label=f"Cluster {cluster_idx+1} ({len(members)} members)")
        ax.add_patch(circle)

        # 클러스터의 점들 그리기
        for member, (x, y) in points.items():
            if member not in plotted_members:
                ax.plot(x, y, 'o', markersize=10,
                       color=cluster_colors[cluster_idx], ...)
                ax.text(x, y, f'  {member}', ...)
                plotted_members.add(member)

    # 가장 큰 원 기준으로 축 범위 설정 ✅
    max_radius = max(c["diameter"]/2.0 for c in clusters)
    margin = max_radius * 0.3
    ax.set_xlim(-max_radius - margin, max_radius + margin)
    ax.set_ylim(-max_radius - margin, max_radius + margin)

    # 범례 추가 ✅
    ax.legend(loc='upper right', fontsize=8)

    # 정보 텍스트
    info_text = f"Total members: {len(all_members)}\nClusters: {len(clusters)}"
    ax.text(0.02, 0.98, info_text, ...)
```

**개선:**
- ✅ 여러 개의 원 그리기
- ✅ 각 클러스터마다 다른 색상
- ✅ 범례로 클러스터 구분
- ✅ 중복 멤버 방지 (첫 등장 위치만 표시)

## 동작 원리

### Diameter 계산

```python
# acc_core.py - decorate_clusters()
for c in clusters:
    members = c["members"]

    # 1) global 덴드로그램에서 sim_global 찾기
    global_sim = find_cluster_in_dendro_by_members(global_dendro, members)

    # 2) 없으면 행렬에서 평균 계산
    if global_sim is None:
        global_sim = average_pairwise_similarity(members, global_matrix)

    c["sim_global"] = global_sim

    # 3) 지름 계산: d = unit / sim_global
    if global_sim <= 0:
        d = unit * 1000
    else:
        d = unit / global_sim  # ← 핵심 공식!

    c["diameter"] = d
    c["theta"] = 180.0 * (1.0 - c["sim_local"])
```

### 예시 계산

```
Sample data (unit=1.0):

Cluster 1: {J, T}
  - sim_global = 0.88
  - diameter = 1.0 / 0.88 = 1.136

Cluster 2: {O, Q}
  - sim_global = 0.83
  - diameter = 1.0 / 0.83 = 1.205

Cluster 3: {J, T, Y}
  - sim_global = 0.78
  - diameter = 1.0 / 0.78 = 1.282

Cluster 4: {N, O, Q}
  - sim_global = 0.70
  - diameter = 1.0 / 0.70 = 1.429

Cluster 5: {J, T, Y, N, O, Q}
  - sim_global = 0.55
  - diameter = 1.0 / 0.55 = 1.818

→ 5개의 동심원, diameter 순서대로!
```

### 시각화

```
        ⭕⭕⭕⭕⭕  ← d=1.818 (sim_global=0.55, 낮음 → 지름 큼)
         ⭕⭕⭕⭕   ← d=1.429 (sim_global=0.70)
          ⭕⭕⭕    ← d=1.282 (sim_global=0.78)
           ⭕⭕     ← d=1.205 (sim_global=0.83)
            ⭕      ← d=1.136 (sim_global=0.88, 높음 → 지름 작음)

핵심:
- sim_global가 높을수록 → 지름 작음 (응집력 높음)
- sim_global가 낮을수록 → 지름 큼 (응집력 낮음)
```

## Text.docx와의 일치

### 논문의 핵심 개념

```
ACC (Area Affinity in Concentric Circles):
1. 두 개의 dendrogram 사용:
   - Local: 특정 시기의 데이터
   - Global: 전체 시기의 데이터

2. 각 cluster는 두 개의 similarity 값:
   - sim_local: closest affinity → angle (θ)
   - sim_global: farthest affinity → diameter (d)

3. Geometric conversion:
   - Diameter: d = unit / sim_global
   - Angle: θ = 180° × (1 - sim_local)

4. Concentric circles:
   - 각 cluster는 고유한 지름의 원을 형성
   - 원들이 동심원 구조로 배치
   - 같은 중심점(origin)을 공유
```

### 구현 일치

| 논문 개념 | 구현 |
|---------|------|
| 두 개의 dendrogram | ✅ `local_dendro`, `global_dendro` |
| sim_local (angle) | ✅ `c["sim_local"]` → `c["theta"]` |
| sim_global (diameter) | ✅ `c["sim_global"]` → `c["diameter"]` |
| d = unit / sim_global | ✅ `d = unit / global_sim` |
| θ = 180°(1 - sim_local) | ✅ `theta = 180.0 * (1.0 - c["sim_local"])` |
| Concentric circles | ✅ 여러 원, 각각 다른 diameter |

## 사용자 경험

### Before

```
사용자: "ACC를 보니까..."
      → "원 하나에 모든 점이 몰려있네?"
      → "왜 동심원이 아니지? 🤔"
      → "논문이랑 다른데?"
```

### After

```
사용자: "ACC를 보니까..."
      → "여러 개의 동심원이 보이네!"
      → "각 클러스터마다 다른 크기의 원!"
      → "논문에 나온 그림이랑 똑같아! ✓"
      → "sim_global가 낮을수록 원이 크구나!"
```

## 기대 효과

### 1. 정확성
- ✅ 논문의 핵심 개념 정확히 구현
- ✅ sim_global 기반 diameter 시각화
- ✅ 각 클러스터의 고유한 특성 표현

### 2. 가독성
- ✅ 클러스터별로 색상 구분
- ✅ 범례로 각 클러스터 식별
- ✅ 멤버 수 정보 표시

### 3. 이해도
- ✅ 클러스터 간의 크기 차이 명확
- ✅ 응집력 높은 클러스터 = 작은 원
- ✅ 응집력 낮은 클러스터 = 큰 원

### 4. 교육/연구
- ✅ 논문 재현 가능
- ✅ 알고리즘 이해 용이
- ✅ 결과 분석 편리

## 테스트 결과

```bash
$ python acc_core.py
ACC all members: {'O', 'N', 'Q', 'Y', 'T', 'J'}

Number of clusters: 5

Cluster details:

  Cluster 1:
    Members: {'T', 'J'}
    Diameter: 1.136
    Theta: 18.00°

  Cluster 2:
    Members: {'O', 'Q'}
    Diameter: 1.205
    Theta: 27.00°

  Cluster 3:
    Members: {'T', 'Y', 'J'}
    Diameter: 1.282
    Theta: 36.00°

  Cluster 4:
    Members: {'O', 'N', 'Q'}
    Diameter: 1.429
    Theta: 45.00°

  Cluster 5:
    Members: {'O', 'Y', 'T', 'N', 'J', 'Q'}
    Diameter: 1.818
    Theta: 72.00°
```

**확인:**
- ✅ 5개의 클러스터 생성
- ✅ 각 클러스터마다 고유한 diameter
- ✅ diameter가 점진적으로 증가 (1.136 → 1.818)
- ✅ 멤버 수가 증가할수록 diameter 증가
- ✅ sim_global 기반 계산 정확

## GUI 테스트

```bash
$ python acc_gui.py
# Load sample_local.csv (local)
# Load sample_global.csv (global)
# Click "Generate ACC"
```

**확인 항목:**
1. ✅ 여러 개의 동심원 표시
2. ✅ 각 원마다 다른 색상
3. ✅ 범례에 클러스터 정보 표시
4. ✅ 원의 크기가 다름 (diameter 반영)
5. ✅ 모든 멤버가 표시됨
6. ✅ 정보 텍스트에 클러스터 수 표시

## 이전 버전 보존

원본 병합 방식이 필요할 경우를 대비하여 `build_acc_merged()` 함수로 보존:

```python
# acc_core.py
def build_acc_merged(local_dendro, global_dendro, global_matrix, unit=1.0):
    """
    Build ACC result by merging all clusters into one
    This is the original implementation that returns a single merged result
    """
    # ... 원본 로직
```

## 결론

ACC 알고리즘의 핵심인 **동심원 구조** 올바르게 구현:

**변경 요약:**
- ✅ `build_acc()`: 다중 클러스터 반환
- ✅ `build_acc_from_matrices()`: 간소화
- ✅ `plot_acc_result()`: 다중 동심원 시각화

**효과:**
- ✅ 논문의 핵심 개념 정확히 구현
- ✅ sim_global 기반 diameter 시각화
- ✅ 각 클러스터의 고유성 표현
- ✅ 사용자 이해도 향상

**사용자가 요구한 "concentric circles" 구조가 완벽하게 구현되었습니다!**
