...
### Step 0: 초기 쌍 배치
- Local matrix에서 가장 높은 유사도를 가진 두 area 선택
- **Radius 계산**: `radius = unit / local_similarity`
  - 유사도가 높을수록 → 반지름이 작음 (더 밀집)
- **Angle 계산**: `angle = 180° × (1 - global_similarity)`
...
**1. find_highest_similarity_pair(local_matrix)**
- 전체 matrix에서 가장 높은 유사도 쌍 찾기

**2. place_first_two_areas(area1, area2, local_sim, global_sim, unit)**
- 첫 두 area를 원 위에 배치
- 반지름과 각도 계산

**3. add_area_to_cluster(cluster, new_area, global_matrix)**
- 기존 클러스터에 단일 area 추가
- 가장 유사한 멤버 기준으로 위치 계산

**4. merge_two_clusters(c1, c2, global_matrix, unit)**
- 두 클러스터를 하나로 병합
- 정렬 멤버(alignment members) 찾기
- 스케일링 및 재배치

**5. find_highest_similarity_with_clusters(local_matrix, global_matrix, placed_areas, clusters)**
- 세 가지 경우 모두 고려하여 다음 최고 유사도 찾기
- 반환: (type, item1, item2, local_sim, global_sim)

**6. build_acc_iterative(local_matrix, global_matrix, unit, method)**
- 전체 알고리즘 실행
- 각 단계의 스냅샷 저장
- 반환: steps 리스트
...
### 반지름 계산
```
radius = unit / local_similarity

예: local_sim = 0.9, unit = 1.0
    → radius = 1.0 / 0.9 = 1.111
```

### 각도 계산
```
angle = 180° × (1 - global_similarity)
