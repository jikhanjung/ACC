...
def build_acc(local_dendro, global_dendro, global_matrix, unit=1.0):
    ...
    # 2) 각 클러스터에 sim_global, diameter, theta 할당
...
1. sim_local (local): 가장 가까운 affinity → angle (θ)
2. sim_global (global): 가장 먼 affinity → diameter (d)
...
Cluster 1: {J, T}
  - sim_local = 0.90 → θ = 18°
  - sim_global = 0.88 → d₁ = 1.0 / 0.88 = 1.136

Cluster 2: {O, Q}
  - sim_local = 0.85 → θ = 27°
  - sim_global = 0.83 → d₂ = 1.0 / 0.83 = 1.205

Cluster 3: {J, T, Y}
  - sim_local = 0.80 → θ = 36°
  - sim_global = 0.78 → d₃ = 1.0 / 0.78 = 1.282
...
```python
def build_acc_multi_circle(local_dendro, global_dendro, global_matrix, unit=1.0):
    """
