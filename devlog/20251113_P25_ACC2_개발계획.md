...
**ACC1 (현재):**
```
diameter = unit / inc_similarity
radius = diameter / 2
```
- 예: inc_sim = 0.88 → diameter = 1.136, radius = 0.568
- 높은 similarity → 작은 원

**ACC2 (신규):**
```
diameter = 1 + (1 - inc_similarity)
radius = diameter / 2
```
- 예: inc_sim = 0.88 → diameter = 1.12, radius = 0.56
...
```
Level 1: J-T merge (inc_sim=0.88)
...
```
Level 3: [J,T]+Y merge (inc_sim=0.81)
...
def analyze_dendrogram_levels(sub_matrix, inc_matrix):
    ...
    # 각 merge 시점마다:
    #   - subordinate similarity 계산
    #   - inclusive similarity 계산
    #   - diameter = 1 + (1 - inc_sim)
    #   - radius = diameter / 2
```
...
```python
def calculate_final_positions(sub_matrix, inc_matrix):
    """
...
2. r=0.560: J-T merge (inc_sim=0.88)
3. r=0.602: O-Q merge (inc_sim=0.83)
4. r=0.595: [J,T]+Y merge (inc_sim=0.81)
5. r=0.655: [O,Q]+N merge (inc_sim=0.69)
6. r=0.824: Final merge (inc_sim=0.353)
