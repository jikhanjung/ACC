...
- **제거된 레이블들**:
  ...
  - "Subordinate Clustering", "Inclusive Clustering" 제목

#### 덴드로그램 헤더 레이아웃 정리
**Before:**
```
Subordinate Clustering
                         [Show similarity values]
```

**After:**
```
Subordinate                [Show similarity values]
```
- 한 줄로 통합: 제목(왼쪽) + 체크박스(오른쪽)
- 공간 절약 및 깔끔한 외관

#### 50:50 레이아웃 분할
- **왼쪽 패널**: Subordinate와 Inclusive 매트릭스가 1:1 비율
- **가운데 패널**: Subordinate와 Inclusive 덴드로그램이 1:1 비율
- `stretch=1` 파라미터 사용
- 구분선 추가:
  ```python
  separator = QLabel()
