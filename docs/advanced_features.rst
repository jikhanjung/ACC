...
NMDS는 유사도/비유사도 행렬을 저차원 공간(2D/3D)에 매핑하는 비모수적 서열화 기법입니다.
ACC의 동심원 시각화와는 다른 관점에서 데이터를 분석할 수 있습니다.
...
사용 가능한 패널: Data, Similarity, Dendrogram, ACC, NMDS
...
지원하는 작업: 셀 값 변경, 다중 셀 일괄 변경, Area/Taxon 추가·삭제·이름 변경, 행/열 재정렬, 테이블 붙여넣기
...
* **Min Diameter**: 동심원의 최소 지름 (기본: 1.0)
* **Max Diameter**: 동심원의 최대 지름 (기본: 2.0)
* **ACC1 Style**: 체크 시 영역을 첫 번째 병합 원 위에 배치 (ACC1 방식)
* **Limit Angle**: 체크 시 각도를 최대 각도 제한 내에 맞춤 (기본: 180°)
