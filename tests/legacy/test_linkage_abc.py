...
print("  Step 0: {A, B} cluster formed with sim_sub=0.9, sim_inc=0.8")
print("  Step 1: Add C to {A, B}")
print("    Linkage similarity (subordinate) = (A-C + B-C) / 2 = (0.5 + 0.5) / 2 = 0.5")
print("    Linkage similarity (inclusive) = (A-C + B-C) / 2 = (0.4 + 0.4) / 2 = 0.4")
...
if len(steps) >= 2:
    ...
    print(f"  Expected subordinate linkage: 0.500")
    print(f"  Actual subordinate linkage: {final_cluster['sub_sim']:.3f}")
    print(f"  Match: {'✅' if abs(final_cluster['sub_sim'] - 0.5) < 0.001 else '❌'}")
    print(f"\n  Expected inclusive linkage: 0.400")
    print(f"  Actual inclusive linkage: {final_cluster['inc_sim']:.3f}")
    print(f"  Match: {'✅' if abs(final_cluster['inc_sim'] - 0.4) < 0.001 else '❌'}")
