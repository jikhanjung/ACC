...
class TestFullPipeline:
    ...
    def test_sample_data_pipeline(self, sample_sub_df, sample_inc_df):
        """샘플 데이터 전체 파이프라인"""
        # 1. DataFrame to dict
        sub_matrix = dict_matrix_from_dataframe(sample_sub_df)
        inc_matrix = dict_matrix_from_dataframe(sample_inc_df)

        # 2. Validate
        valid_sub, msg_sub = validate_similarity_matrix(sample_sub_df.values)
        valid_inc, msg_inc = validate_similarity_matrix(sample_inc_df.values)

        assert valid_sub, f"Subordinate validation failed: {msg_sub}"
        assert valid_inc, f"Inclusive validation failed: {msg_inc}"
    ...
    def test_simple_abc_pipeline(self, simple_abc_sub_matrix, simple_abc_inc_matrix):
        """간단한 3-영역 파이프라인"""
        result = build_acc_from_matrices(
            simple_abc_sub_matrix,
            simple_abc_inc_matrix,
            unit=1.0,
            method='average'
        )
    ...
    def test_step_by_step_pipeline(self, simple_abc_sub_matrix, simple_abc_inc_matrix):
        """단계별 ACC 생성"""
    ...
    def test_iterative_algorithm(self, simple_abc_sub_matrix, simple_abc_inc_matrix):
        """반복적 알고리즘 (ACC2)"""
...
class TestDifferentMethods:
    ...
    def test_clustering_methods(self, simple_abc_sub_matrix, simple_abc_inc_matrix, method):
    ...
    def test_ward_method_not_applicable(self, simple_abc_sub_matrix, simple_abc_inc_matrix):
...
class TestEdgeCases:
    ...
    def test_unit_parameter(self, simple_abc_sub_matrix, simple_abc_inc_matrix):
        """다양한 unit 값 테스트"""
        units = [0.5, 1.0, 2.0]
...
class TestResultStructure:
    ...
    def test_result_has_required_fields(self, simple_abc_sub_matrix, simple_abc_inc_matrix):
    ...
    def test_cluster_structure(self, simple_abc_sub_matrix, simple_abc_inc_matrix):
