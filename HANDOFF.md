...
| 테스트 | 126 케이스 (unit/integration/gui) |
| CI/CD | GitHub Actions 7개 워크플로우 |
| 린팅 | Ruff 0.13+ (pre-commit hooks) |
| 문서 | v0.0.6 기준 최신 상태 |
| 빌드 | PyInstaller + InnoSetup (Windows) |
...
| `acc_core_tree.py` | ~290 | **신규** 트리 기반 ACC (ACCNode, build/render 분리) |
| `acc_gui.py` | ~3,900 | PyQt5 GUI 애플리케이션 (ACC2 제거됨) |
| `acc_utils.py` | ~550 | 유틸리티 (similarity↔distance, tree 래퍼) |
